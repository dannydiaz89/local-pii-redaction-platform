...
describe('capability manifest and preflight', () => {
  it('accepts a valid, internally consistent manifest', () => {
    expect(() => { assertCapabilityManifest(capabilityManifest(), 'cor_synthetic_manifest_001'); }).not.toThrow();
  });
  ...
  it('rejects internally inconsistent capability references', () => {
    ...
    expect(() => { assertCapabilityManifest(manifest, 'cor_synthetic_manifest_002'); }).toThrow(SafeError);
  });
  ...
  it('rejects verifier-to-format references that are not declared by the format', () => {
    ...
    manifest.verificationProfiles.push({ ...manifest.verificationProfiles[0], id: 'alternate-profile' });
    expect(() => { assertCapabilityManifest(manifest, 'cor_synthetic_manifest_003'); }).toThrow(SafeError);
  });
  ...
  it('rejects a per-format byte limit above the deployment limit', () => {
    ...
    expect(() => { assertCapabilityManifest(manifest, 'cor_synthetic_manifest_005'); }).toThrow(SafeError);
  });

  it('rejects an available model detector in rules-only mode', () => {
    const manifest = capabilityManifest();
    manifest.detectors[0].kinds = ['MODEL'];
    expect(() => { assertCapabilityManifest(manifest, 'cor_synthetic_manifest_006'); }).toThrow(SafeError);
  });
});
